# Israeli Scholarship Quick-Reference Table

A consolidated lookup for the major Israeli higher-education scholarships referenced by this skill. Use as a fast filter before deep-diving into any specific track. All amounts are illustrative and refresh annually, always verify on the source URL listed in the SKILL.md Reference Links table.

## Universal-eligibility (most students should check)

| Scholarship | Profile filter | Typical grant | Application timing |
|---|---|---|---|
| PEREACH (פר"ח) | Any undergraduate | NIS 7,000 (~114 hrs) or NIS 10,000 (140 hrs) | תשפ"ז cycle: opens September, first-come-first-served until ~November (NOT May-September) |
| MilGo (MoE socioeconomic) | BA / hand-engineer / mechina at MALAG/MAHAT institution | Tier A NIS 12,480 / B NIS 10,000 / C NIS 7,500 / D NIS 5,000 (4-tier structure) | Cycle opens December via Milgapo |
| University merit at admission | Incoming freshman with strong bagrut + psychometric | One-time grant per institution; HUJI/TAU/Technion vary | Automatic at registration |

## Post-service (IDF + Sherut Leumi)

| Scholarship | Profile filter | Typical grant | Application timing |
|---|---|---|---|
| Pikadon Hayashi | Discharged soldier or Sherut Leumi alum, within post-discharge window (5 years for most; verify current window for active reservists/lone soldiers) | The user's own deposit | Anytime within window |
| MoD periphery scholarship | Lived in priority-area yishuv 5 of 6 years before studies, within the post-discharge window (5 years standard, 10 years for active reservists / lone soldiers) | Up to 100% first-year BA tuition (~NIS 11,653, תשפ"ה/2025) | Annual; cannot stack with other periphery |
| Mimadim LiLimudim | Combat veterans + special populations (large annual cohort) | **GRADUATED**: 85% Years 1-2, top-up to 100% in final year, capped NIS 10,214.45/year (below NIS 12,017 baseline tuition) | תשפ"ו: opens 3.11.2025, closes 31.7.2026, document submission by 30.8.2026 |
| Atid Foundation | Combat veterans (foundation track) | Per-track grant | Check current cycle |
| IMPACT! (FIDF) | Former combat / combat-support soldiers from low-income backgrounds | USD 4,000/year for up to 4 years; ~130 community-service hours/year + 2 donor letters/year | Annual; apply via FIDF |

## Iron Swords reservist framework (CHE + MoD, תשפ"ה-תשפ"ז)

| Threshold | Profile filter | Grant | Notes |
|---|---|---|---|
| 8+ cumulative reserve days from 7.10.2023 | Any reservist student | NIS 1,100 one-time | Universal floor |
| Reservist parent of children under 14 | Any reservist student with eligible children | NIS 2,000 one-time | Stacks on top of NIS 1,100 |
| 50+ days reserve service under Order 8 | Reservist students (combat array) | One-time tuition grant up to ~NIS 11,653 (2025 cohort) | Check che.org.il/war/ for current cohort amount + window |
| Combat-track extended reserve service | Combat reservists | Higher-tier tuition support, up to full tuition | Cannot stack with Mimadim LiLimudim same year |
| 61-99 reserve days | Any reservist student | 8 credit-unit exemption (electives + general) | Academic accommodation |
| 100+ reserve days | Any reservist student | 10 credit-unit exemption | Same |

## Statutory tracks for bereaved families, wounded soldiers, evacuees

| Track | Profile filter | Grant | Application channel |
|---|---|---|---|
| יתומי מערכות ישראל / bereaved siblings | Statutory under חוק משפחות חיילים שנספו במערכה | Tuition support + stipends | Misrad HaBitachon Family & Commemoration Branch (NOT hachvana portal) |
| נכי צה"ל | Statutory under חוק הנכים | Tuition + rehabilitation support | Rehabilitation Branch (אגף השיקום) via case manager |
| Evacuee (מפונים) tracks | Residents of עוטף עזה / north evacuated since 7.10.2023 who became students 2024-2026 | TAU emergency fund (~NIS 2,500/student); Heznek La'Atid; university-by-university funds | Per-institution + foundation portals |

## Olim chadashim

| Scholarship | Profile filter | Typical grant | Application timing |
|---|---|---|---|
| Student Authority (Minhal HaStudentim) | Oleh Chadash; post-army eligibility window now 5 years regular / 7 years career service | Partial/full tuition | תשפ"ו deadline: 31.7.2026; documents 30.8.2026 |
| Repatriate "Masa Harhava" | Within 3 years of aliyah | Up to ~90% tuition discount | Annual |

## Foundation grants

| Scholarship | Profile filter | Typical grant | Application timing |
|---|---|---|---|
| Rashi Foundation Katzir | University/college students, periphery focus | Tuition scholarship + living stipend; amount not published | Annual |
| Rashi Foundation Rakia | Periphery + excellence | Amount not published | Annual |
| Rashi single-mother track | Single mothers | Amount not published | Annual |
| ISEF (in Israel) | Underserved-background, strong academics | 400-500 scholarships a year on 20 campuses | Annual |
| ISEF (international) | MD/PhD/postdoc abroad | Cohort size varies by year | Annual |

## Graduate-level (MA / PhD / postdoc)

| Scholarship | Profile filter | Typical grant | Application timing |
|---|---|---|---|
| Adams Fellowship | 2nd-year PhDs in **exact sciences, mathematics, life sciences ONLY** (NOT humanities/social sciences) | Full tuition + NIS 100,000/year + USD 3,000/year travel; 3 years base + up to 1 extension | Opens October-November, closes early January for following year |
| Aharon Katzir Postdoctoral Fellowship | Postdoc in humanities or social sciences | Multi-year postdoc-abroad track via Israel Academy of Sciences | Per-cycle; check academy.ac.il |
| PBC / MALAG outstanding-doctoral-from-periphery | PhDs from periphery yishuvim | Council for Higher Education direct track | Per-cohort |
| Maof program (PBC/MALAG) | Outstanding young scientists from Arab community | Faculty-integration pipeline | Per-cohort |
| Rothschild Fellowship (Yad Hanadiv) | Postdoc abroad after PhD from Israeli university | USD 75,000/year **ceiling on OTHER external funding** (exceeding it disqualifies, not just caps) | Annual |

## Specialized tracks

| Scholarship | Profile filter | Notes |
|---|---|---|
| HUJI free first-year tuition for women in CS/Math | Female student entering CS or Math at HUJI | Automatic on enrollment |
| Weizmann promotes-women-in-science | Women doctoral / postdoctoral | About 10 doctoral seats per year |
| Mifal HaPais athlete track | Israeli national team representative | National-team verification required |
| University faculty-specific awards | All graduate students | Pattern only, check faculty page |
| PEREACH miluim supplement | PEREACH recipient who served reserve duty during academic year | Stacks on top of base PEREACH AND on top of Iron Swords floor |

## Stacking rules (cross-cutting)

- MoD periphery scholarship: cannot stack with another periphery scholarship.
- Iron Swords tuition support and Mimadim LiLimudim: cannot be received in the same academic year.
- Iron Swords NIS 1,100 + NIS 2,000 child supplement: stacks with PEREACH, MilGo, Pikadon, PEREACH miluim supplement.
- Rothschild USD 75,000/year: ceiling on OTHER external funding (not the Rothschild itself).
- Rashi / ISEF: must declare all received grants in application; under-declaration risks revocation.
