...
### Step 1: Understand the Bagrut System Structure
The Bagrut (בגרות) is Israel's national matriculation exam system, administered by the Ministry of Education (Misrad HaChinuch). Students take exams during their final years of high school (tikhon), typically grades 10-12.
...
- Mathematics (Matematika) at minimum 3 units
- English (Anglit) at minimum 3 units
- Hebrew Language and Literature (Lashon v'Sifrut) at minimum 2 units
- Bible Studies (Tanakh) at minimum 2 units
- History (Historia) or Civics (Ezrachut) at minimum 2 units
- At least one subject at 5-unit level (any subject)

### Step 2: Bagrut Scoring Formula
...
- Final Grade = (External Exam Score x 0.70) + (School Grade x 0.30)
- Passing threshold: 56% (ציון עובר)
- Students who fail may retake exams (moed bet, typically in the following testing period)
...
### Step 3: Psychometric Test (PET) Structure
...
- 8 sections total (some are pilot sections that don't count)
- Multiple choice with 4 answer options
- No penalty for wrong answers (guessing is encouraged)
- Available in Hebrew, Arabic, Russian, French, Spanish, and combined Hebrew-English

### Step 4: Registration and Test Dates
...
- Universities typically accept the highest score achieved
- No waiting period between attempts (can take every test date)
- Some students take 2-3 times to improve their score
- Preparation courses (mechinot) are common and widely available

### Step 5: Study Planning Strategies
...
### Step 6: University Admission (Sekher Calculation)
...
1. Calculate your Bagrut weighted average (memutza bagrut meshuklal)
2. Add bonus points for 5-unit and 4-unit subjects
3. Convert to the university's scale
4. Apply the university's specific formula combining Bagrut and PET
5. Compare to published admission thresholds (saf kabala) for your desired program

**Key tips for maximizing Sekher:**
- A high PET score can compensate for a lower Bagrut average
- Bonus points from 5-unit subjects significantly boost the Bagrut component
- Some programs accept either PET or Bagrut alone if one is exceptionally high
- Check each university's specific calculation; formulas differ significantly

## Examples

### Example 1: Bagrut Grade Calculation
**Input:** "I got 78 on my Math bagrut external exam and my magen is 85. What's my final grade?"
**Output:** Calculate using the 70/30 formula: Final Grade = (78 x 0.70) + (85 x 0.30) = 54.6 + 25.5 = 80.1. The final Bagrut grade in Math is 80. Explain that this is above the passing threshold of 56%, and if taken at 5-unit level, bonus points will be added by universities when calculating the Sekher.

### Example 2: PET Score Interpretation
**Input:** "I got 650 on the Psychometric, is that good enough for law school?"
...
### Example 3: Study Unit Planning
**Input:** "I'm in 10th grade, how should I choose my 5-unit subjects?"
...
### Example 4: PET Registration Process
**Input:** "How do I sign up for the September Psychometric?"
**Output:** Walk through the process: Visit NITE website (www.nite.org.il), registration typically opens June-July for September test date. Need Teudat Zehut number, fee of approximately 500-600 NIS. Choose test language and test center location. If requesting special accommodations (hatkamot), submit documentation well in advance (may require 2-3 months lead time). After registration, NITE sends confirmation with exact test center and time.

## Bundled Resources

### References
- `references/bagrut-subjects.md` -- Complete list of Bagrut subjects with study unit levels (3/4/5), mandatory vs. elective classification, bonus points by subject, and minimum requirements for full certificate. Consult when a user asks which subjects are required, what unit levels mean, or how bonus points work.
- `references/psychometric-preparation.md` -- PET preparation strategies, recommended study materials, practice test sources, preparation timelines, and score improvement tips by current score range. Consult when a user asks how to prepare for the Psychometric, which study materials to use, or how to improve their score.

### Scripts
- `scripts/sekher-calculator.py` -- Calculates the university admission weighted score (sekher) given Bagrut average and Psychometric score. Supports formulas for Hebrew University, Tel Aviv University, Technion, and Ben-Gurion University. Run: `python scripts/sekher-calculator.py --help`

## Troubleshooting

### Error: "Failed a Bagrut exam (below 56%)"
Cause: Score fell below the minimum passing threshold of 56%.
Solution: The student can retake the exam in the next testing period (moed bet). The retake replaces the original score if higher. Continue attending school to maintain a strong magen (school grade) which contributes 30% of the final grade. Consider tutoring (shi'urim prati'im) for the specific subject. Contact the school guidance counselor (yo'etz chinuchi) for support options.

### Error: "PET score lower than expected"
Cause: Test anxiety, insufficient preparation, or unfamiliarity with the exam format.
Solution: Students can retake the PET at any of the four annual test dates with no limit on attempts. Universities accept the highest score. Consider enrolling in a preparation course (mechina psichometrit) if self-study was insufficient. Use NITE's official practice materials to identify weak sections. Focus preparation on the weakest section, as improvement there yields the largest score gain.

### Error: "Unsure which university will accept my Sekher"
Cause: Each university has different admission formulas and thresholds that change annually.
Solution: Check each university's admissions website for published thresholds from the previous year (safey kabala). Use the university's online Sekher calculator if available. Contact the university admissions office (minhal rishum) directly for guidance. Consider applying to multiple universities and programs to maximize acceptance chances. Remember that thresholds published are from the previous year and may shift.
